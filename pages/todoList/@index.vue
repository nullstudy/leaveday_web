...
export default {
  ...
  async created() { //vue 인스턴스가 생성된 후에 실행됨
    var user_id = this.$store.getters.userInfo._id;
    var a = this.getTodoList(this.$store.getters.userInfo._id);
    // console.log('create',this.$store.state)
    this.selectedTab = this.tabs[0]; //todo 탭 선택


    let test = this.$store.state;
    console.log(test) // {...}
    console.log(test.todoList) // null
    console.log('test',test && test.todoList)

    // let item = this.$store.state.todoList
    this.totalTodo = [];
  ...
  mounted(){
    console.log('before',this.$store.state.todoList);
  },
  data() { 
    return {
      value: null,
